...
int main(int argc, char **argv) {
 ...
 /* Construction de l'adresse locale (pour bind) */
  ...
  /*
  struct ip_mreqn groupe;
  struct in_addr inp;
  inet_aton("224.2.2.77",&inp);
  groupe.imr_multiaddr = inp;
  groupe.imr_address.s_addr = INADDR_ANY;
  groupe.imr_ifindex = 0;
  if(setsockopt(sfd,IPPROTO_IP,IP_ADD_MEMBERSHIP,&groupe,sizeof(groupe))==0){
    printf("IP_ADD_MEMBERSHIP success with multicast %s\n","224.2.2.77");
  }else
