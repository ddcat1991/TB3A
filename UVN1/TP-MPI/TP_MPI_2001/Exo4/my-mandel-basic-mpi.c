...
int main(int argc, char *argv[])
{
  ...
  if (rank == 0) {
   ...
   for(j = -MAXY; j <= MAXY; j++) {
    ...
    for(j= -MAXY; j <= MAXY; j++){
        MPI_Recv(&res, NX, MPI_INT, 1, DATATAG, MPI_COMM_WORLD, &status);
        for(i = -MAXX; i <= MAXX; i++) {
            cases[i + MAXX][j + MAXY] = res[i + MAXX];
        }
    }
    ...
    for(j= -MAXY; j<= MAXY; j++){
        ...
        for(i= -MAXX; i<=MAXX; i++){
            x = 2 * i / (double)MAXX;
            y = 1.5 * j / (double)MAXY;
            res_lign[i+MAXX] = mandel(x, y);
        }
        //printf("child %d send a lign\n",rank);
        MPI_Send(&res_lign, NX , MPI_INT, 0, DATATAG, MPI_COMM_WORLD);
    }
